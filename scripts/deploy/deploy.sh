...
echo "========================================"
echo ""
echo -e "${GREEN}[i] Приложение (HTTP): http://localhost:8081${NC}"
echo -e "${GREEN}[i] Приложение (HTTPS): https://localhost:8443${NC}"
echo -e "${GREEN}[i] Страница тренажёра: https://localhost:8443/index${NC}"
echo -e "${GREEN}[i] Панель преподавателя: https://localhost:8443/teacher${NC}"
echo -e "${GREEN}[i] Пароль преподавателя: teacher123${NC}"
echo -e "${YELLOW}[i] ВНИМАНИЕ: HTTPS использует самоподписанный сертификат, браузер покажет предупреждение — нажмите «Продолжить»${NC}"
echo ""